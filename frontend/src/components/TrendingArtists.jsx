        ...
        {artists.length === 0 ? (
          ...
          artists.map((artist, index) => (
            <motion.div
              ...
              className="group flex items-center space-x-4 p-4 gradient-border bg-gradient-to-r from-gray-800/60 to-gray-900/60 rounded-xl hover:from-gray-700/70 hover:to-gray-800/70 cursor-pointer relative overflow-hidden"
            >
              {/* Hover glow effect */}
              ...
              <div className="flex-1 min-w-0 relative z-10">
                ...
                <div className="flex items-center space-x-4 text-xs text-gray-300 mb-2">
                  <span className="font-medium">Score: <span className="text-white">{artist.trend_score?.toFixed(1) || 0}</span></span>
                ...
                {artist.genres && artist.genres.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {artist.genres.slice(0, 3).map((genre, i) => (
                      <span
                        key={i}
                        className="text-xs px-2.5 py-1 bg-gradient-to-r from-gray-700/50 to-gray-800/50 text-gray-200 rounded-full border border-gray-600/30 font-medium"
                      >
                        {genre}
                      </span>
                    ))}
              ...
              <div className="flex-shrink-0 text-right text-xs space-y-1 relative z-10">
                <motion.div
                  className="flex items-center justify-end space-x-1 text-gray-300"
